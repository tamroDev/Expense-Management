...
import { updateRevenu } from "../../../../redux/reduxSlices/revenuSlice";

function UpdateForm({ cancel, id }) {
  const dispatch = useDispatch();
  const { revenuList } = useSelector((state) => state.revenu);
  const revenuItem = revenuList.filter((item) => item._id === id);

  const [revenu, setRevenu] = useState({
    budget: revenuItem[0].budget,
    category: revenuItem[0].category,
    dateEnd: revenuItem[0].dateEnd,
    description: revenuItem[0].description,
  });

  const logId = () => {
    dispatch(updateRevenu({ id, revenu }));
    cancel(false);
  };
  return (
